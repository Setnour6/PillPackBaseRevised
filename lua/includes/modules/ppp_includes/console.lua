...
pk_pills.convars.noclip = CreateConVar("pk_pill_allownoclip", 0, FCVAR_REPLICATED, "Makes player able to exit pill when they press the noclip button.")
pk_pills.convars.admin_delayattack = CreateConVar("pk_pill_admin_delayattack", 1, FCVAR_REPLICATED, "Allows attacking upon first entering a pill for some time.")
...
pk_pills.convars.admin_globalspeed = CreateConVar("pk_pill_admin_globalspeed", 1, FCVAR_REPLICATED, "Whether all pills from any pill pack have the same walk and run speed.")
pk_pills.convars.admin_globalspeedamount = CreateConVar("pk_pill_admin_globalspeedamount", 540, FCVAR_REPLICATED, "Amount of run speed that all pills will have if pk_pill_admin_globalspeed is enabled.", 1, 2000)
pk_pills.convars.admin_globalcrouchwalkspeed = CreateConVar("pk_pill_admin_globalcrouchwalkspeed", 1, FCVAR_REPLICATED, "Whether all pills from any pill pack have the same crouch walk and run speed.")
pk_pills.convars.admin_globalcrouchwalkspeedamount = CreateConVar("pk_pill_admin_globalcrouchwalkspeedamount", 0.4, FCVAR_REPLICATED, "Amount of crouch walk speed from all pills will have if pk_pill_admin_globalcrouchwalkspeed is enabled.", 0.1, 1)
pk_pills.convars.admin_globaljumppower = CreateConVar("pk_pill_admin_globaljumppower", 1, FCVAR_REPLICATED, "Whether all pills from any pill pack have the same jump power.")
pk_pills.convars.admin_globaljumppoweramount = CreateConVar("pk_pill_admin_globaljumppoweramount", 4, FCVAR_REPLICATED, "Amount of jump power that all pills will have if pk_pill_admin_globaljumppower is enabled.", 1, 100)
pk_pills.convars.admin_attackschangespeed = CreateConVar("pk_pill_admin_attackschangespeed", 1, FCVAR_REPLICATED, "Whether pills get a bit slower for a moment after attacking.")
...
pk_pills.convars.admin_fastcloakamount = CreateConVar("pk_pill_admin_fastcloakamount", 15, FCVAR_REPLICATED, "How fast cloaking is for some pills if pk_pill_admin_fastcloak is enabled.", 1, 255)
