...
if SERVER then
    ...
    concommand.Add("pk_pill_apply_full", function(ply, cmd, args, str)
        if ply:IsSuperAdmin() then
            apply(ply, args[1], "user", tonumber(args[2]))
    ...
    concommand.Add("pk_pill_restore_full", function(ply, cmd, args, str)
        if ply:IsSuperAdmin() then
            restore(ply)
            ply:SetNWInt("badguy", false)
            table.remove(nextbots_tbl, ply)
            if ply:GetNWInt("AftonH") ~= 3 then
    ...
    concommand.Add("pk_pill_restore_force_full", function(ply, cmd, args, str)
        if not ply:IsAdmin() or ply:IsSuperAdmin() then return end
        if ply:IsSuperAdmin() then
            restore(ply, true)
            ply:SetNWInt("badguy", false)
